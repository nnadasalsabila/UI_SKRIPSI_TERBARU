...
with tab_stasioneritas:
    if 'data' in locals() and data is not None and not data.empty and "Harga" in data.columns:
        ...
        # Step 1 - Uji ADF awal
        if st.button("Jalankan Uji ADF (Data Asli)"):
            ...
            if result[1] <= 0.05:
                ...
                data_diff = data["Harga"].diff().dropna()  # tetap siapkan data_diff untuk ACF/PACF
            else:
                ...
                if st.button("Lakukan Differencing"):
                    ...
                    # Step 3 - Uji ADF setelah differencing
                    if st.button("Uji ADF Setelah Differencing"):
